...
void ConvolutionOnDevice(const Matrix M, const Matrix N, Matrix P)
{
    ...
    int gridX = P.width/TILE_SIZE;
    if(P.width % TILE_SIZE != 0){ // If P.width is not even multiple of TILE_WIDTH
      gridX++; // P.width is even multiple of TILE_WIDTH
    }

    int gridY = P.height/TILE_SIZE;
    ...
    //printf("The time it takes to run: %3.1f ms\n", time);
