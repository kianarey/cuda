...
uint32_t *convertTo1D(uint32_t** input){
    
  uint32_t *temp = (uint32_t*)malloc(INPUT_WIDTH * INPUT_HEIGHT * sizeof(uint32_t));
  //convert row-wise; code from matrixmul
    for(int i = 0; i < INPUT_HEIGHT; i++){
        for(int j = 0; j < INPUT_WIDTH; j++){
            temp[i * INPUT_WIDTH + j] = input[i][j];
        }
    }
    return temp;
}
...
int main(int argc, char* argv[])
{
    ...
    uint32_t *d_input, *temp = NULL;
    cudaMalloc((void **) &d_bins, HISTO_WIDTH * HISTO_HEIGHT * sizeof(unsigned int));
    cudaMalloc((void **) &d_input, INPUT_WIDTH * INPUT_HEIGHT * sizeof(uint32_t));
    
    /* End of setup code */
    
    temp = convertTo1D(input);
    cudaMemcpy(d_input, temp, INPUT_WIDTH * INPUT_HEIGHT * sizeof(uint32_t), cudaMemcpyHostToDevice);
    
    /* This is the call you will use to time your parallel implementation */
    TIME_IT("opt_2dhisto",
            50,
            opt_2dhisto(d_input, INPUT_HEIGHT * INPUT_WIDTH, d_bins);)

    /* Include your teardown code below (temporary variables, function calls, etc.) */
    cudaMemcpy(kernel_bins, d_bins, HISTO_WIDTH * HISTO_HEIGHT * sizeof(unsigned int), cudaMemcpyDeviceToHost);
    cudaFree(d_input);
    cudaFree(d_bins);
