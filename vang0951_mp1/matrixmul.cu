...
int main(int argc, char** argv) {
    ...
    //for (int i = 0; i < MATRIX_SIZE; i++){
      //printf("i = %d, %f, %f, %f\n", M.elements[i], N.elements[i], P.elements[i]);
    //}
...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
  ...
  // First allocate device matrix
  Matrix M_d = AllocateDeviceMatrix(M);
  Matrix N_d = AllocateDeviceMatrix(N);
  Matrix P_d = AllocateDeviceMatrix(P);

  // Copy host matrices to device M_d and N_d
  CopyToDeviceMatrix(M_d, M);
  CopyToDeviceMatrix(N_d, N);

  // Get dimBlock and dimGrid sizes
  dim3 dimBlock(MATRIX_SIZE, MATRIX_SIZE);
  dim3 dimGrid(1, 1); // can the number of threads per block be anything?

  // Launch kernal function
  MatrixMulKernel<<<dimGrid, dimBlock>>>(M_d, N_d, P_d);

  // Sync
  cudaDeviceSynchronize();

  // Copy from device matrix to host P
  CopyFromDeviceMatrix(P, P_d); // but P_d isn't constant?

  // Free pointers to elements in matrices
